 ...
 *
 * This class resembles <tt>std::any</tt> but additional supports extensions via
 * the \ref Any::Base interface. The Mitsuba Property class uses \ref Any to
 * store objects that aren't part of the supported set of property types, such
 * as Dr.Jit tensor objects or other specialized types. The additional features
 * compared to <tt>std::any</tt> enable seamless use in Python bindings. Any
 * values are copyable with reference semantics.
 */
class Any {
public:
...
template <typename T> T *any_cast(Any *a) {
    return (a && a->p && a->p->type() == typeid(T)) ?
